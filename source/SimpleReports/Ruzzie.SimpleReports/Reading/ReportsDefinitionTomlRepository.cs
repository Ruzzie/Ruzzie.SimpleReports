...
public class ReportsDefinitionTomlRepository : IReportsDefinitionRepository
{
        ...
        foreach (var reportId in tomlDoc.Keys)
        {
            ...
            if (tomlValue.TryReadTomlTable(out var reportTable))
            {
                var reportDefinition =
                    ParseReportDefinitionTable(reportId
                                             , reportTable
                                             , listProviderTypeResolver
                                                                   ...
                                                                   postProcessPipelineTypeResolver)
    {
        ...
        var report = new ReportDefinition
                     {
                         Id          = id, Title = reportTable.ReadStringValueOr("title", string.Empty)
                                                           ...
                                                           , ITypeResolver<IListProvider> typeResolver)
    {
        ...
        var fieldType = Enum.Parse<ParameterFieldType>(paramTable.ReadStringValueOr("field_type", string.Empty), true);

        switch (parameterType)
        {
            case ParameterType.DATE_RANGE:
                return ParseDateRangeParameter(parameterId, paramTable, fieldType, parameterType, typeResolver);
